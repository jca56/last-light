...
fn default_tavern() -> tavern::TavernData {
    tavern::TavernData::new()
}
...
impl GameState {
    ...
    pub fn update(&mut self, data: &GameData) -> Vec<GameEvent> {
        ...
        // ── Tavern ────────────────────────────────────────
        self.tick_tavern(&mut events);

        self.last_updated = now();
        events
    }

    fn tick_tavern(&mut self, events: &mut Vec<GameEvent>) {
        let now_ts = now();

        // 1. Check for visitor arrivals
        if now_ts >= self.tavern.next_visitor_at
            && self.tavern.occupancy() < self.tavern.capacity()
        {
            let visitor = tavern::generate_visitor(self.tavern.reputation);
            events.push(GameEvent::TavernVisitorArrived {
                name: visitor.name.clone(),
            });
            self.tavern.visitors.push(visitor);
            self.tavern.schedule_next_visitor();
        }

        // 2. Auto-stock: try to serve waiting visitors from inventory
        for visitor in self.tavern.visitors.iter_mut() {
            if visitor.is_fully_served() || visitor.is_patience_expired() {
                continue;
            }
            // Try to serve food
            if let Some(ref food_id) = visitor.food_order {
                if !visitor.food_served && self.inventory.remove(food_id, 1) {
                    visitor.food_served = true;
                }
            }
            // Try to serve drink
            if let Some(ref drink_id) = visitor.drink_order {
                if !visitor.drink_served && self.inventory.remove(drink_id, 1) {
                    visitor.drink_served = true;
                }
            }
        }

        // 3. Process completed visitors (served or patience expired)
        let mut served_indices = Vec::new();
        let mut left_indices = Vec::new();
        for (i, visitor) in self.tavern.visitors.iter().enumerate() {
            if visitor.is_fully_served() {
                served_indices.push(i);
            } else if visitor.is_patience_expired() {
                left_indices.push(i);
            }
        }

        // Process served visitors (earn gold)
        for &i in served_indices.iter().rev() {
            let visitor = self.tavern.visitors.remove(i);
            let food_bonus = if visitor.food_served {
                self.tavern.food_multiplier()
            } else {
                1.0
            };
            let drink_bonus = if visitor.drink_served {
                self.tavern.drink_multiplier()
            } else {
                1.0
            };
            let gold = (visitor.gold_reward as f64 * food_bonus.max(drink_bonus)) as u32;
            self.gold = self.gold.saturating_add(gold);
            self.tavern.total_gold_earned += gold;
            self.tavern.total_served += 1;
            self.tavern.reputation = self.tavern.reputation.saturating_add(1);
            events.push(GameEvent::TavernVisitorServed {
                name: visitor.name,
                gold,
            });
        }

        // Process departed visitors (reputation penalty)
        for &i in left_indices.iter().rev() {
            if i < self.tavern.visitors.len() {
                let visitor = self.tavern.visitors.remove(i);
                self.tavern.reputation = self.tavern.reputation.saturating_sub(1);
                events.push(GameEvent::TavernVisitorLeft {
                    name: visitor.name,
                });
            }
        }

        // 4. Passive room income (1 gold per room every ~60 seconds)
        // We check this by using last_updated as a reference
        if self.tavern.upgrades.rooms > 0 {
            let elapsed = now_ts.saturating_sub(self.last_updated);
            // Approximate: if more than 60s passed, grant room income
            if elapsed >= 60_000 {
                let room_gold = self.tavern.upgrades.rooms;
                self.gold = self.gold.saturating_add(room_gold);
                if room_gold > 0 {
                    events.push(GameEvent::TavernRoomIncome { gold: room_gold });
                }
            }
        }
    }
