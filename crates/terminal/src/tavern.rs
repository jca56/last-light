    ...
    loop {
        ...
        for event in events {
            match event {
                ...
                game::GameEvent::TavernVisitorArrived { name } => {
                    state.log_messages.push((
                        format!("{} walks into the tavern.", name),
                        Style::default().fg(ui::WARM_WHITE),
                    ));
                    state.auto_scroll(20);
                }
                game::GameEvent::TavernVisitorServed { name, gold } => {
                    state.log_messages.push((
                        format!("{} pays {} gold and leaves satisfied.", name, gold),
                        Style::default().fg(ui::GOLD),
                    ));
                    state.auto_scroll(20);
                }
                game::GameEvent::TavernVisitorLeft { name } => {
                    state.log_messages.push((
                        format!("{} leaves unhappy — nothing to serve.", name),
                        Style::default().fg(ui::EMBER),
                    ));
                    state.auto_scroll(20);
                }
                game::GameEvent::TavernRoomIncome { gold } => {
                    state.log_messages.push((
                        format!("Room rent collected: {} gold.", gold),
                        Style::default().fg(ui::GOLD),
                    ));
                    state.auto_scroll(20);
                }
